...
- [ ] 2026-09-01 → implementation: verify the replacement link-only contract in an ordinary ChatGPT Project chat. Each selected message must require one `get_media` call, return exactly one `resource_link`, remain below 32 KiB of serialized MCP output, materialize once after approval, and leave a fresh conversation usable after photo, video-contact-sheet, and voice tests.
...
- 2026-09-01 — ordinary-ChatGPT acceptance invalidated the inline-media assumption: the chosen photo, contact sheet, and voice payloads serialized to about 3.08 million base64 characters; the 1,877,990-byte voice note alone became 2,503,988 characters and also carried a duplicate `resource_link`. A fresh chat immediately reached its maximum length while file materialization repeated.
- 2026-09-01 — approved requirement change: every successful `get_media` media result is link-only. The MCP call plans one best representation and returns compact metadata plus exactly one short-lived `resource_link`; photos/images link to a model-friendly image, videos/GIFs/video notes to a JPEG contact sheet, voice/audio to original source bytes, and documents to the best original or preview. Direct `image`, `audio`, embedded-resource, and base64 content are prohibited. The existing original route stays lightweight; a separate heavy view route materializes normalized images and contact sheets. No external object store is added unless deployed evidence later requires an explicit amendment.
