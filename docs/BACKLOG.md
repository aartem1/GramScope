...
## Multi-source identity: Sorq

**Decision:** if/when the project is promoted from Telegram-specific GramScope to a multi-source information tool, the product will be renamed **Sorq**. Naming exploration is considered complete; do not spend more work searching for alternative names unless a concrete blocking conflict appears before migration.

**Meaning:** `Sorq` is a short coined name loosely combining the ideas of **source** and **query**. It is intentionally source-neutral and fits the broader concept:

> agent access to information streams: search, discovery, reading and monitoring across multiple sources.

The name should remain valid for Telegram and Threads as well as possible future sources such as RSS, Bluesky, GitHub or Hacker News.

### Rename scope

When the rename is executed, treat it as a complete product migration from **GramScope → Sorq**, not just a display-name change. Rename/update all current product-level surfaces where practical, including:

- GitHub repository name and description;
- package/app identifiers that refer to the overall product;
- README and current architecture/product documentation;
- MCP server/plugin name, description and presentation metadata;
- deployment/service name;
- public URL/domain;
- OAuth redirect URLs and provider configuration tied to the public URL;
- product-level environment/config names where a rename improves clarity;
- external links, badges and setup instructions;
- names used for new branches/tasks/specs after the migration.

Telegram-specific identifiers and variables should stay Telegram-specific rather than being mechanically renamed to Sorq.

Do **not** rewrite Git history or historical commits merely to erase the GramScope name. Old design documents may remain historical unless they are actively used as current product documentation. Preserve or redirect the old public URL where practical so existing integrations do not break unnecessarily.

### Timing

Do not perform the rename as part of the current Telegram work. Execute it when multi-source development becomes real — ideally when the second source is being promoted from experiment to supported functionality. At that point, use **Sorq** directly rather than reopening naming exploration.
