 ...
 * just as unusable for a peer the account lacks. Its exception clause is not
 * a restatement of `manage_folder`'s description: a shared default stated here
 * has to name the one action it does not cover, or this text sends the model
 * at the one call that rejects a @username.
 *
...
export const SERVER_INSTRUCTIONS = [
  ...
  "Addressing sources: Name a source by @username whenever it has one. A marked id like -1001234567890 resolves only for chats this account belongs to, so it is not a durable handle for a public channel reached by search or by link. The exception is manage_folder(remove_sources), which takes the marked ids list_folders reports and rejects a @username.",
